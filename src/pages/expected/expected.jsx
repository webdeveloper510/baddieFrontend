  ...
  const handleSubmit = async (values, action) => {
    ...
    try {
      ...
      const body = {
        ...
        player_id: playerType === "pitcher" ? pitcher.pitcher[parseInt(values.playerName)] : batter.batter[parseInt(values.playerName)],
    ...
    <div className="flex flex-col items-center w-[100%] bg-lightblack  px-4 min-w-full" >
      ...
      <Formik innerRef={formikRef} initialValues={initialValues} validationSchema={validationSchema} onSubmit={handleSubmit}>
          {({ isSubmitting, setFieldValue, values }) => (
            <Form>
              <div className={`grid ${!evalData?"grid-cols-5":"grid-cols-6"} gap-4 player-flex text-[#2a3f5f]`}>
                <div className="text-center col-span-2 player-select">
                  ...
                  >
                    ...
                    {['Ks', 'Hits Allowed', 'Walks', 'Outs'].map((item, i) => (
                ...
                <div className="text-center col-span-2 player-select">
                  <div className="flex items-center justify-center pt-5 pb-5">
                    <div className="relative group w-full" ref={dropdownRef}>
                      <button type="button" onClick={toggleDropdown} className=" py-3 px-3 player-list buttonPlayer  text-center focus:outline-none appearance-none inline-flex justify-center w-full px-4  text-sm font-medium text-gray-700  border border-gray-300 rounded-md shadow-sm bg-white ">
                      ...
                      {values.playerType != "" && <div className={`absolute right-0 mt-2 rounded-md shadow-lg  ring-1 ring-black ring-opacity-5 p-1 space-y-1 ${isOpen ? '' : 'hidden'} max-h-60 bg-white w-full`}>
              ...
              {evalData &&  <div className="text-center col-span-1  player-select">
                  ...
                  <Field as="select"
                    className="py-3 px-3 my-5 self-center player-list rounded w-full text-center focus:outline-none appearance-none"
                    name="time_frame">
          ...
          <div className="flex player-flex justify-between mt-20">
            <div className="w-96 player-select flex justify-center">
              <div className="w-full">
                <div className="text-center">
                  <span className="text-2xl font-bold">{metricType} {range}</span>
                </div>
            ...
            <div className="text-center w-96 text-[#2a3f5f]  player-select">
              <select className="bg-lightgray player-list selector-mobile w-full  greater py-3 px-3 px-5 rounded mt-6" value={equality} onChange={e => setEquality(e.target.value)}>
            ...
            <div className="flex items-center justify-center">
              <button onClick={handleSubmitOutsideFormik} className="bg-black text-white px-5 my-4 py-2 rounded">
                run
              </button>
            </div>
          </div>
